...
async function updateTopRoles(guild) {
  try {
    ...
    if (!cfg.topRoles || typeof cfg.topRoles !== 'object') {
      console.log('[TOP ROLES] ⚠️ No hay topRoles configurados');
      return;
    }
    ...
    if (Object.keys(guildLevels).length === 0) {
      console.log('[TOP ROLES] ⚠️ No hay datos de niveles para este servidor');
      return;
    }
    ...
    console.log('[TOP ROLES] 📊 Top 5 usuarios:', sortedUsers.slice(0, 5).map((u, i) => `${i+1}. <@${u.id}> (Lv${u.level}, ${u.xp} XP)`));
    ...
    for (const [posStr, roleId] of Object.entries(cfg.topRoles)) {
      ...
      if (!desiredUser) {
        console.log(`[TOP ROLES] ⚠️ No hay usuario en posición ${pos}`);
        continue;
      }
      ...
      console.log(`[TOP ROLES] 🎯 Procesando Top ${pos} → ${role.name} para <@${desiredUser.id}> (Lv${desiredUser.level}, ${desiredUser.xp} XP)`);
      ...
      if (!member) {
        console.log(`[TOP ROLES] ⚠️ Usuario ${desiredUser.id} no encontrado en el servidor`);
        continue;
      }
      ...
      if (!member.roles.cache.has(roleId)) {
        ...
        console.log(`[TOP ROLES] ✅ ${member.user.username} ya tiene Top ${pos}`);
      }
    }
