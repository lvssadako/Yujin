...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      await member.kick(`${reason} — por ${interaction.user.tag}`).catch(err => { throw err; });

      return interaction.reply({ content: `✅ Expulsado: ${member.user.tag} (${member.id})\nRazón: ${reason}` });
    } catch (err) {
