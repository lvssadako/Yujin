...
// (Lógica básica, botones y PvP pueden agregarse después)
...
async function mostrarTurno(interaction, partida) {
  ...
  if (!partida.prefixMsgId && interaction.reply && !interaction.deferred && !interaction.replied) {
    ...
    // Si ya se respondió, edita el mensaje
...
async function terminarPartida(interaction, partida, motivo) {
  ...
  let monto = 0;
  if (valJ > 21) {
    resultado = 'Te pasaste. ¡Perdiste!'; color = 0xdd2e44;
    ...
    monto = partida.apuesta;
    resultado = `¡Ganaste! (+${monto} 🪙)`; color = 0x43b581;
    ...
    monto = -partida.apuesta;
    resultado = `El bot gana. ¡Perdiste! (-${partida.apuesta} 🪙)`; color = 0xdd2e44;
    u.gambleStats.losses = (u.gambleStats.losses || 0) + 1;
    u.gambleStats.lossesToday = (u.gambleStats.lossesToday || 0) + partida.apuesta;
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      (monto !== 0 ? `\n\n${monto > 0 ? 'Ganancia' : 'Pérdida'}: ${monto > 0 ? '+' : ''}${monto} 🪙` : '')
    )
...
async function terminarPartidaAbandono(interaction, partida) {
  ...
  const embed = new EmbedBuilder()
    .setColor(0xdd2e44)
    .setTitle('♠️ Blackjack - Abandono')
    .setDescription(
      `Partida cancelada por inactividad. Pierdes la apuesta de ${partida.apuesta} 🪙.`
    )
    .setFooter({ text: 'Blackjack vs Bot' });
  let msgId = partida.prefixMsgId;
  let channelId = partida.prefixChannelId;
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // El mensaje será público y editable
    ...
    if (left > 0) {
      ...
      if (typeof interaction.reply === 'function') {
        await interaction.reply({ content: `⏳ Espera ${sec}s para volver a apostar.`, ephemeral: false });
      } else if (interaction.message && typeof interaction.message.reply === 'function') {
        await interaction.message.reply(`⏳ Espera ${sec}s para volver a apostar.`);
      }
      return;
    }
    if (bet == null || bet < MIN_BET) {
      if (typeof interaction.reply === 'function') {
        await interaction.reply({ content: `❌ La apuesta mínima es ${MIN_BET}.`, ephemeral: false });
      } else if (interaction.message && typeof interaction.message.reply === 'function') {
        await interaction.message.reply(`❌ La apuesta mínima es ${MIN_BET}.`);
      }
      return;
    }
    ...
    if (coins < bet) {
      if (typeof interaction.reply === 'function') {
        await interaction.reply({ content: `❌ Fondos insuficientes. Tienes ${coins} 🪙.`, ephemeral: false });
      } else if (interaction.message && typeof interaction.message.reply === 'function') {
        await interaction.message.reply(`❌ Fondos insuficientes. Tienes ${coins} 🪙.`);
      }
      return;
    }
    // Sin límite diario de pérdidas: se mantiene el comportamiento original del juego sin alterar la experiencia
    if (!removeCoins(guildId, userId, bet)) {
      if (typeof interaction.reply === 'function') {
        await interaction.reply({ content: '❌ No se pudo procesar la apuesta.', ephemeral: false });
      } else if (interaction.message && typeof interaction.message.reply === 'function') {
        await interaction.message.reply('❌ No se pudo procesar la apuesta.');
      }
      return;
    }
    ...
    if (global.blackjackGames[userId]) {
      if (typeof interaction.reply === 'function') {
        await interaction.reply({ content: '❗ Ya tienes una partida de blackjack en curso. Usa los botones para continuar.', ephemeral: false });
      } else if (interaction.message && typeof interaction.message.reply === 'function') {
        await interaction.message.reply('❗ Ya tienes una partida de blackjack en curso. Usa los botones para continuar.');
      }
      return;
    }
    ...
    // Timeout de abandono: cancela la partida si no hay interacción en 1 minuto
