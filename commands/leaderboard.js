...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('leaderboard')
    .setDescription('🏆 Mira quienes son los 10 usuarios mas activos'),
  ...
  async execute(interaction, client) {
    ...
    const all = readLevels()[guildId] || {};
    const arr = Object.entries(all)
      .map(([id, d]) => ({ id, level: d.level || 0, xp: d.xp || 0 }))
      .sort((a,b) => (b.level - a.level) || (b.xp - a.xp))
      .slice(0, topN);

    if (arr.length === 0) return interaction.editReply('No hay datos de niveles todavía.');
    ...
    ctx.fillText(`Top ${arr.length} usuarios mas activos`, panelX + headerPad, panelY + headerPad + 48);
