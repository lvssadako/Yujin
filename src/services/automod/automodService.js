...
function pruneSpamCache(now) {
  if (spamCache.size > 100) {
    for (const [id, timestamps] of spamCache.entries()) {
      if (!timestamps.length || now - timestamps[timestamps.length - 1] > CACHE_TTL_MS) {
        spamCache.delete(id);
      }
    }
  }
...
module.exports = {
  clearSpamCache: () => spamCache.clear(),
  getSpamCacheSize: () => spamCache.size,
