...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(s => s.setName('delreward').setDescription('Eliminar reward para un nivel')
      .addIntegerOption(o => o.setName('level').setDescription('Nivel').setRequired(true)))
    .addSubcommand(s => s.setName('setrolexp').setDescription('Configura bonus de XP para un rol (ej: 50 = +50%)')
      .addRoleOption(o => o.setName('role').setDescription('Rol').setRequired(true))
      .addNumberOption(o => o.setName('bonus').setDescription('Bonus en porcentaje (ej: 50 = +50%, máx 1000)').setRequired(true)))
  ...
  async execute(interaction) {
    ...
    try {
      ...
      // ✅ FIX: Asegurar estructura correcta
      ...
      async function applyRewardsAndNotify(userId, oldLevel, newLevel) {
        ...
        if (newLevel > oldLevel) {
          ...
          if (cfg.levelUpChannelId) {
            const validation = await validateChannelForSending(guild, cfg.levelUpChannelId);
            if (validation.valid) {
              notifyChannel = validation.channel;
            } else {
              console.warn(`[leveladmin] Level-up channel validation failed: ${validation.reason}`);
            }
          }

          let content = `:tada: <@${userId}> subió al nivel **${newLevel}**!`;
          const gained = [];
      ...
      if (sub === 'setrolexp') {
        ...
        // ✅ FIX: Validar ANTES de convertir
        if (typeof bonus !== 'number' || isNaN(bonus) || bonus < 0 || bonus > 1000) {
          return interaction.editReply({ 
            content: '❌ Valor inválido. Usa **número entero** (ej: 50 = +50%). Rango: 0-1000%', 
            ephemeral: true 
          });
        }
        ...
        cfg.roleXpBonuses[role.id] = bonusDecimal;
        writeCfg(cfg);

        return interaction.editReply({ 
          content: `✅ Bonus configurado: ${role} → **+${bonus}%** de XP`, 
          ephemeral: true 
        });
      }
      ...
      if (sub === 'listrolexp') {
        ...
        const lines = entries.map(([rid, bonusDecimal]) => {
          const role = interaction.guild.roles.cache.get(rid);
          const percent = (Number(bonusDecimal) * 100).toFixed(0);
          return `${role ? `${role}` : rid} — **+${percent}%**`;
        });
        return interaction.editReply({ content: `**📊 Bonuses de XP por Rol:**\n${lines.join('\n')}`, ephemeral: true });
      }
      ...
      if (sub === 'setreward') {
        ...
        const validation = validateRoleForAssignment(interaction.guild, role.id, 'level reward');
        if (!validation.valid) {
      ...
      if (sub === 'delreward') {
        ...
        if (cfg.levelRewards[String(lvl)]) {
          ...
          writeCfg(cfg);
          return interaction.editReply({ content: `✅ Recompensa eliminada para nivel ${lvl}`, ephemeral: true });
        } else {
      ...
      if (sub === 'givexp' || sub === 'takexp' || sub === 'reset') {
        ...
        // ✅ FIX: Inicializar con estructura completa
        const current = levels.guilds[guildId][user.id] || { 
          xp: 0, 
          level: 0, 
          messages: 0, 
          voiceMinutes: 0 
        ...
        if (sub === 'givexp') {
          ...
          // ✅ FIX: Mantener todos los campos
          levels.guilds[guildId][user.id] = {
            ...current,
            xp: newXp,
            level: newLevel
          };
          ...
          return interaction.editReply({ 
            content: `✅ **+${amount} XP** otorgado a ${user}\nNivel: **${oldLevel}** → **${newLevel}** (XP: ${newXp}/${xpToNext(newLevel)})`, 
            ephemeral: true 
          });
        }
        ...
        if (sub === 'takexp') {
          ...
          return interaction.editReply({ 
            content: `✅ **-${amount} XP** quitado a ${user}\nNivel: **${oldLevel}** → **${newLevel}** (XP: ${newXp}/${xpToNext(newLevel)})`, 
            ephemeral: true 
          });
        }
        ...
        if (sub === 'reset') {
          ...
          writeLevels(levels);
          return interaction.editReply({ content: `✅ Datos de niveles reseteados para ${user}`, ephemeral: true });
        }
      }
